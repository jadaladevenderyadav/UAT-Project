...
import saveFacilityRequest from "@salesforce/apex/revaApprovedFacilityController.saveFacilityRequest";
import getTimeValues from "@salesforce/apex/revaApprovedFacilityController.getTimeValues";
import getEventFacilities from "@salesforce/apex/revaApprovedFacilityController.getEventFacilities";
import getLabFacilities from "@salesforce/apex/revaApprovedFacilityController.getLabFacilities";
import getCourseScedules from "@salesforce/apex/revaApprovedFacilityController.getCourseScedules";
import getAllTimeSlots from "@salesforce/apex/revaApprovedFacilityController.getAllTimeSlots";
import getFilteredFacilities from "@salesforce/apex/revaApprovedFacilityController.getFilteredFacilities"

export default class RevaApprovedFacility extends NavigationMixin(LightningElement) {
  // @api recordId;
  @track recordId;
 ...
 @track approvedFacilityStatus = [];
 @track approvalData;
 @track facilityDataId;
 @track facilityDataId;
  @track facilityElectiveStatus;
...
get computedPlaceholder() {
  ...
  async handleChange(event) {
    ...
    switch (name) {
      ...
      case "iaExamHall":
        if( this.selectedFacilityType==='Classroom' ){
            this.selectedFacilityType='Classroom';
        }
        if(this.selectedFacilityType==='Laboratory'){
            this.selectedFacilityType='Laboratory';
        }
  ...
  async getEventFacilities() {
    ...
    async getLabFacilities() {
      ...
      // Facility type name
        ...
        const responseData = res.map((facility, ind) => ({
          label: facility.Facility_Type__c ? facility.Facility_Type__c : 'NA',
          value: facility.Facility_Type__c ? facility.Facility_Type__c : 'NA', 
        }));
        const uniqueFacilityOptionsSet = new Set(
          responseData.map(JSON.stringify)
        );
 ...
 async getFilteredFacilities(accountId, facilityType, buildingName, floorNumber ,facilityName) {
    ...
    if(facilityType !== null && buildingName === null && floorNumber === null && facilityName === null){
        ...
        const facilityBlockData = res.map((facility, ind) => ({
          label: facility.Building_Name__c ? facility.Building_Name__c : 'NA',
          value: facility.Building_Name__c ? facility.Building_Name__c : 'NA' ,
          }));
    ...
    if(facilityType !== null && buildingName !== null && floorNumber !== null && facilityName === null){
      ...
      // room number
          ...
          label: facility.Room_No__c ? facility.Room_No__c : 'NA',
          value: facility.Room_No__c ? facility.Room_No__c : 'NA',
          }));
    ...
    if(facilityType !== null && buildingName !== null && floorNumber !== null && facilityName !== null){
      ...
      const approvedFacility = res.map((facility,ind)=>({
        label: facility.ApprovalStatus__c ? facility.ApprovalStatus__c : 0,
        value: facility.ApprovalStatus__c ? facility.ApprovalStatus__c : 0,
      }));
      const facilitycApprovalStatus = new Set(
        approvedFacility.map(JSON.stringify)
      );
      this.approvedFacilityStatus = Array.from(facilitycApprovalStatus).map(
        JSON.parse
      );
      this.approvalData = this.approvedFacilityStatus.map((e)=>e.label) ? this.approvedFacilityStatus.map((e)=>e.label): '';
      console.log('approval data ---->',JSON.stringify(this.approvalData))

      const facilityId = res.map((facility,ind)=>({
        label: facility.Id ? facility.Id : 0,
        value: facility.Id ? facility.Id : 0,
      }));
      const facilityIds = new Set(
        facilityId.map(JSON.stringify)
      );
      this.facilityIdData = Array.from(facilityIds).map(
        JSON.parse
      );
      this.facilityDataId = this.facilityIdData.map((e)=>e.label) ? this.facilityIdData.map((e)=>e.label): '';
      console.log('facility id --->' ,JSON.stringify(this.facilityDataId))
    }

          const capacityOptions = res.map((facility,ind)=>({
            label: facility.Capacity__c ? facility.Capacity__c : 0,
            value: facility.Capacity__c ? facility.Capacity__c : 0,
          }));
  ...
  async getCourseScedules(){
    await getCourseScedules({recordId:this.recordId}).then((res)=>{
      ...
      res.map((e,ind)=>{
        ...
        this.facilityElectiveStatus=e.hed__Course_Offering__r.Active_Elective_Course__c
      })
            return '';
    }).catch((err)=>{
 ...
 // facilityType: this.selectedFacilityType ? this.selectedFacilityType : this.facilityTypes ? this.facilityTypes : this.eventValue,
        ...
        approvalStatus:this.approvalData ? this.approvalData[0] : '',
        id:this.facilityDataId ? this.facilityDataId[0] : '',
        facilityElectiveStatus:this.facilityElectiveStatus ? this.facilityElectiveStatus:false
      };
